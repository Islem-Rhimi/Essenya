...
import { RecivedReservationTableRow } from "./ReservationTableRow";

interface RecivedReservationTableProps {
  eventId: string;
}

export const RecivedReservationTable: React.FC<
  RecivedReservationTableProps
> = ({ eventId }) => {
  const { status } = useSession();
  const { paginationStates } = usePagination();

  const { data, isLoading, error } =
    api.reservation.getReservationByEventId.useQuery(
      {
    ...
    <div className="flex-col space-y-6">
      <Card className="overflow-auto rounded-xl border border-[rgba(212,175,55,0.3)] p-0 shadow backdrop-blur">
        <Table>
          <TableHeader>
            <TableRow>
              <TableHead className="px-4 py-3 pl-8">Utilisateur</TableHead>
              <TableHead className="px-4 py-3">Téléphone</TableHead>
