...
export function DeleteProductModal({
  ...
  const deleteproduitMutation = api.produits.delete.useMutation({
    onSuccess: async () => {
  ...
  const handleDelete = () => {
    deleteproduitMutation.mutate({ id: produitId });
  };
      ...
      <DialogContent className="max-h-[90vh] max-w-4xl overflow-y-auto p-0">
        <DialogHeader className="bg-produitd p-6 pb-0">
          ...
          <DialogDescription>
            ...
            <span className="font-semibold text-yellow-500">{produitName}</span>{" "}
            ?
            <br />
